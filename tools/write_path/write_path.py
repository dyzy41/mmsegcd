...
dataset_path = "/home/user/dsj_files/CDdata/LEVIR-CD/cut_data"  # 将此处替换为您的数据集路径
output_path = "/home/user/dsj_code/mmsegcd/mmsegmentation/data/cd"  # 将此处替换为您想要保存txt文件的输出路径
...
for folder_name in ["train", "val", "test"]:
    ...
    with open(txt_path, "w") as f:
        ...
        for image_fileA, image_fileB, label_file in zip(image_filesA, image_filesB, label_files):
            ...
            label_path = os.path.join(label_folder, label_file)
            f.write(f"{image_pathA}  {image_pathA}  {label_path}\n")
